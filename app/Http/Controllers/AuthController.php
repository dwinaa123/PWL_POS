...
{
     //kita buat validasi saat tombol login di klik
     ...
     if (Auth::attempt($credential)) {
        //kalau berhasil simpan data user yang di variable $user
        $user = Auth::user();
        //cek login jika level user admin maka arahkan ke halaman admin
        if ($user -> level_id == '1') {
            //dd($user->level_id);
            return redirect()->intended('admin');
        }
...
public function proses_register(Request $request)
{
    ...
    //validasi username itu harus unique atau tidak boleh duplicate username nya
    ...
    //jika berhasil isi level dan passwordnya ya biar secure
    $request['level_id'] = '2';
